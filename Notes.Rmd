...
This post also uses a funnel plot function, for indirectly standardised ratios, that I built as part of my PhD work.  The function is based on `ggplot2` [@wickhamGgplot2ElegantGraphics2009], and is available at https://github.com/chrismainey/CMFunnels, although it's a work in progress.  
...
```{r basicfunnel, warning=FALSE, error=FALSE, message=FALSE}
library(ggplot2)
library(tidyr)
...
```{r, funnel1, message=FALSE, fig.align='center', fig.retina=5, collapse=TRUE, warning=FALSE}
funnel_plot(numerator=medpar$los, denominator=medpar$prds, group = medpar$provnum, title = 'Length of Stay Funnel plot for `medpar` data', 
...
funnel_plot(numerator=medpar$los, denominator=medpar$prds, group = medpar$provnum,  title = 'Length of Stay Funnel plot for `medpar` data', 
            Poisson_limits = FALSE, OD_adjust = TRUE, method = "SHMI",
            label_outliers = 99, return_elements = "plot")
```
...
If you'd like to use my outline `ggplot` function, or contribute, please pull or fork it on github: https://github.com/chrismainey/FunnelPlotR, or install from CRAN with `install.packages('FunnelPlotR')`.
